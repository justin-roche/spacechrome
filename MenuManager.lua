...
function MenuManager:populateMenus()
    ...
    for menuName, menuConfig in pairs(self.menuList) do
        ...
        self:createMenu(menuName,
                        ...
                        menuConfig.enterFunction)
    end
end

----------------------------------------------------------------------------------------------------
-- Create menu
function MenuManager:addMinorMode(menuName, menuItems)
  -- add a minor menu option to the main menu
  if menuName == "chromeMenu" then
    table.insert(menuItems, 
                 {cons.cat.action, '', ',', "minor mode", {
                    {cons.act.func,
                     function() 
                       print("state", devtoolsState)
                       self:switchMenu(devtoolsState)
    end}}})
  end
end
...
function MenuManager:createMenu(menuName,
    ...
    print("Creating menu: " .. menuName, menuItems)

    self:addMinorMode(menuName, menuItems)
...
function MenuManager:switchMenu(menuName)
    ...
    print("")
    print("Switching to insert keyboard")
    normalKeyboard()
...
function MenuManager:showApplicationMenu(menuName)
  app = hs.application.frontmostApplication()
  name = app:name()
  if menuName == 'mainMenu' then
    if name == 'Google Chrome' then
      print("setting application menu")
      return "chromeMenu"
    end
  end
  return menuName
...
function MenuManager:openMenu(menuName)
    ...
    print("Showing menu " .. menuName)
    menuName = self:showApplicationMenu(menuName)
